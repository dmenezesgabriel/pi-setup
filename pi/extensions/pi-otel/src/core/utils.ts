...
export function redactText(value: string | undefined | null, patterns?: RegExp[]): string | undefined {
  if (value == null) return undefined;
  const s = String(value);
  // default patterns: email and simple phone-like numbers
  const defaultPatterns: RegExp[] = [
    /[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,}/gi,
    /\+?[0-9]{1,3}[\s-]?\(?[0-9]{2,4}\)?[\s-]?[0-9\s-]{5,}/g,
  ];
  const all = [...defaultPatterns, ...(patterns ?? [])];
  let out = s;
  for (const pat of all) out = out.replace(pat, '***REDACTED***');
  return out;
}
...
function extractMessageArray(payload: any): any[] | undefined {
  // If the payload is already an array of messages, return it
  if (Array.isArray(payload)) return payload;
  // If it's a JSON string that encodes an array, try to parse it
