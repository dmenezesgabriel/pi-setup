...
describe('index bootstrap', () => {
  it('registers handlers and shuts down SDK on session_shutdown', async () => {
    ...
    // extension initialized log should have been emitted
    expect(pi.logger.info).toHaveBeenCalledWith('pi-otel-mlflow: extension initialized');
    ...
    // call the handlers and expect shutdown to be invoked via otel.shutdownSdk and logged
    for (const h of handlersArray) await h();
    expect((otelModule.shutdownSdk as any)).toHaveBeenCalled();
    expect(pi.logger.info).toHaveBeenCalledWith('pi-otel-mlflow: OpenTelemetry SDK shut down');
  });
});
